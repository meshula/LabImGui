...
void lab_imgui_window_state(const char* window_name, lab_WindowState * s)
{
    ...
    memset(s, 0, sizeof(lab_WindowState));
    s->width = sapp_width();
...
extern "C"
void lab_imgui_new_docking_frame(lab_WindowState* ws)
{
    if (!ws->valid)
        return;
    ...
    // The ImGuiWindowFlags_NoDocking prevents docking into the parent window,
    ...
    // ImGuiDockNodeFlags_PassthruCentralNode indicates that DockSpace() will 
    // render the background and handle the pass-thru hole, so we ask Begin() 
    // to not render a background.
    if (dockspace_flags & ImGuiDockNodeFlags_PassthruCentralNode)
        window_flags |= ImGuiWindowFlags_NoBackground;

    // Important: we proceed even if Begin() returns false
    // (aka window is collapsed).
    // This is to keep the DockSpace() active. If a DockSpace() is inactive, 
    // all active windows docked into it lose their parent and become undocked.
    // The docking relationship between an active window and an inactive docking
    // window cannot be preserved, otherwise 
...
#if 0
            // split the dockspace into 2 nodes -- DockBuilderSplitNode takes 
            // in the following args in the following order
            //   window ID to split, direction, fraction (between 0 and 1), 
            //   the final two setting lets us choose which id we want (which 
            //   ever one we DON'T set as NULL, will be returned by the function)
            //                                                              
            //  out_id_at_dir is the id of the node in the direction we 
            //  specified earlier, out_id_at_opposite_dir is in the opposite direction
            auto dock_id_left = ImGui::DockBuilderSplitNode(dockspace_id, 
                    ImGuiDir_Left, 0.2f, nullptr, &dockspace_id);
            auto dock_id_down = ImGui::DockBuilderSplitNode(dockspace_id,
    ...
    
    {
        ...
        ImGui::Begin("Canvas Window", nullptr, window_flags);
        bool click_released = ImGui::InvisibleButton("full screen.1", 
                                                     viewport->Size);

        static bool c_clicked = false;

        ws->canvas_hovered = ImGui::IsItemHovered();
        bool active = ImGui::IsItemActive();
        ws->canvas_click_started = active && !c_clicked;
        ws->canvas_click_ended = click_released;
        ws->canvas_click_active = active;
        c_clicked = active;
        ImVec2 mouse_pos = io.MousePos - ImGui::GetCurrentWindow()->Pos;
        ws->canvas_x = mouse_pos.x;
        ws->canvas_y = mouse_pos.y;
        ImGui::End();
        ImGui::PopStyleVar();
        ImGui::PopStyleVar();
 
        static bool dock_window = true;
        ImGuiDockNode* central = ImGui::DockBuilderGetCentralNode(dockspace_id);
        if (dock_window && central) {
            ImGui::DockBuilderDockWindow("YOWZERS", central->ID); 
            dock_window = false;
        }
    }
}
...
static void init(void) {
    ...
    desc_t.max_vertices = 20 * 1024 * 1024;
    desc_t.max_commands = 1 * 1024 * 1024;
    sgl_setup(&desc_t);
