...
// https://github.com/cdsama/SquarifiedTreemap/
...
struct Rectangle
{
    ...
    Rectangle scale(Number Ratio) const
    {
    ...
    Rectangle scale(Pair<Orientation, Number> v) const
    {
...
struct Layout
{
    ...
    Layout *AddData(Number Data)
    {
        if (CurrentDatas.empty())
        {
            ...
            Rectangle NewData = Container.scale(Data / TotalData);
        ...
        else
        {
            Number FirstRatio = Zero;
            {
                ...
                Rectangle CurrentBlock = Container.scale(CurrentTotal / TotalData);
                Rectangle LastData = CurrentBlock.scale(MakePair(DataOrientation, (CurrentDatas.back() / CurrentTotal)));
            ...
            UniquePtr<Layout> MayBeNextLayout = MakeUnique<Layout>();
            {
                ...
                Rectangle NewData = MayBeNextLayout->Container.scale(Data / MayBeNextLayout->TotalData);
    ...
    Array<Rectangle> GetLayoutRectangles() const
    {
        ...
        Rectangle LastBlock = Container.scale(LastTotal / TotalData);
        for (auto &&i : CurrentDatas)
        {
            Rectangle ThisBlock = LastBlock.scale(MakePair(DataOrientation, i / LastTotal));
